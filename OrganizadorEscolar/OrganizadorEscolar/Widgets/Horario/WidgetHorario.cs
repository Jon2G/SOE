...
namespace OrganizadorEscolar.Widgets.Horario
{
    public abstract class WidgetHorario
    {
        ...
        public Dia Actual { get; protected set; }
        ...
        public void CargarDia(Dia actual)
        {
            Materias.Clear();
            ...
            for (int i = 0; i < 5; i++)
            {
                Materias.Add(new OrganizadorEscolar.Widgets.Horario.Materia(horarios[i], materias[(j + i) % 5], colores[(j + i) % 5]));
            }
