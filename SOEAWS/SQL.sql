...
VALUES ('SUBJECT_COLOR_10','#3ad4e8','#0b3136')
INSERT INTO APP_COLORS (IDENTIFIER,COLOR,DARK_COLOR) 
VALUES ('SUBJECT_COLOR_11','#5bab16','#2f540f')
INSERT INTO APP_COLORS (IDENTIFIER,COLOR,DARK_COLOR) 
...
BEGIN
	...
	BEGIN
		INSERT INTO INSCRIPTIONS(TEACHER_SUBJECTS_ID,USER_ID,GROUP_ID,SUBJECT_ID) VALUES (@TEACHER_SUBJECTS_ID,@USER_ID,@GROUP_ID,@SUBJECT_ID)
	END
	ELSE IF (SELECT SUBJECT_ID FROM INSCRIPTIONS WHERE USER_ID=@USER_ID AND GROUP_ID=@GROUP_ID AND @TEACHER_SUBJECTS_ID=TEACHER_SUBJECTS_ID ) IS NULL
	BEGIN
		UPDATE INSCRIPTIONS SET SUBJECT_ID=@SUBJECT_ID WHERE USER_ID=@USER_ID AND GROUP_ID=@GROUP_ID AND @TEACHER_SUBJECTS_ID=TEACHER_SUBJECTS_ID 
	END
...
END
GO
ALTER PROCEDURE SP_UPDATE_GRADES
(@PARTIAL INT,
@NUMERIC_SCORE INT,
...
IF @USER_ID<=0
BEGIN
	SELECT 'INVALID_REQUEST','NO USER'
	RETURN
END
...
IF @GROUP_ID<=0
BEGIN
	SELECT 'INVALID_REQUEST','NO GROUP'
	RETURN
END

DECLARE @INSCRIPTION_ID INT=0

IF @SUBJECT_NAME IS NULL BEGIN
...
BEGIN
	...
	IF @SUBJECT_ID<=0
	BEGIN
		SELECT 'INVALID_REQUEST','INVALID SUBJECT'
	RETURN
	END
	...
	IF @INSCRIPTION_ID<=0
	BEGIN
		UPDATE INSCRIPTIONS SET SUBJECT_ID=@SUBJECT_ID WHERE USER_ID=@USER_ID AND GROUP_ID=@GROUP_ID 
		SELECT @INSCRIPTION_ID=SCOPE_IDENTITY();
	END
END

IF @INSCRIPTION_ID<=0
BEGIN
	SELECT 'INVALID_REQUEST','NO INSCRIPTION ID [{'+CAST(@USER_ID AS varchar)+'}'+'{'+CAST(@GROUP_ID AS varchar)+'}{'+CAST(@SUBJECT_ID AS varchar)+'}]'
	RETURN
END
...
SELECT ID AS 'ID',@GROUP_ID,NUMERIC_SCORE,TEXT_SCORE,PARTIAL,@SUBJECT_ID FROM GRADES WHERE ID=@GRADE_ID
END
GO
...
AS
BEGIN
DELETE fROM SCHOOLCONTACT WHERE USER_ID=@USER_ID
DELETE FROM SUBJECT_COMMENTS WHERE USER_ID=@USER_ID
DELETE FROM LINKS_REPORTS WHERE USER_ID=@USER_ID
DELETE fROM LINKS WHERE INSCRIPTION_ID IN (SELECT ID FROM INSCRIPTIONS WHERE USER_ID=@USER_ID)
DELETE fROM REMINDER WHERE INSCRIPTION_ID IN (SELECT ID FROM INSCRIPTIONS WHERE USER_ID=@USER_ID)
DELETE fROM TODO WHERE INSCRIPTION_ID IN (SELECT ID FROM INSCRIPTIONS WHERE USER_ID=@USER_ID)
