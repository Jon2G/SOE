...
namespace SOEWeb.Components
{
    ...
    public partial class InfiniteScrolling<T> : IList<T>
    {
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender)
            {
                this._module = await JSRuntime.InvokeAsync<IJSObjectReference>("import", "./InfiniteScrolling.js");
                this._currentComponentReference = DotNetObjectReference.Create(this);
                this._instance = await this._module.InvokeAsync<IJSObjectReference>("initialize", this._lastItemIndicator, this._currentComponentReference);
            }
        }
